...
// Reproduction from the warrantum dogfood: an annotation that wrapped onto
// indented continuation lines ended collection, so every entry after it was
// dropped from the contract without a warning. Two root files were declared
// three times, never parsed, and reported as scope violations each time —
// three review cycles chasing a phantom.
func TestParseOwnedPathsWrappedAnnotation(t *testing.T) {
	content := "- **owned_paths:**\n" +
		"  - authority/adapters/shim/\n" +
		"  - authority/.gitignore — `adapters/.venv/`; the pre-draft said the\n" +
		"    root `.gitignore`, but `__pycache__/` is already covered at\n" +
		"    `authority/.gitignore:57`, so the entries belong beside it\n" +
		"  - OVERVIEW.md — pre-slice planning commits only, no diff here\n" +
		"  - AGENTS.md — regenerated by `metis surface`, no diff here\n" +
		"- **read_only_paths:** everything else\n"
	got, warnings := ParseOwnedPathsWithWarnings(content)
	want := []string{"authority/adapters/shim/", "authority/.gitignore", "OVERVIEW.md", "AGENTS.md"}
	if len(got) != len(want) {
		t.Fatalf("parse = %v, want %v", got, want)
	}
	for i := range want {
	...
	if len(warnings) != 0 {
		t.Errorf("wrapped annotation should not warn, got %v", warnings)
	}
}

// The continuation rule must not swallow the lines that genuinely end the
// list, or scope would silently widen to whatever bullets follow the brief.
func TestParseOwnedPathsTerminators(t *testing.T) {
	for name, content := range map[string]string{
		"field bullet": "- **owned_paths:**\n" +
			"  - src/ — a wrapped\n    annotation\n" +
			"- **read_only_paths:** internal/\n" +
			"  - not/a/path\n",
		"heading": "- **owned_paths:**\n" +
			"  - src/ — a wrapped\n    annotation\n" +
			"\n## Definition of Done\n\n- not/a/path\n",
		"unindented prose": "- **owned_paths:**\n" +
			"  - src/ — a wrapped\n    annotation\n" +
			"Prose at column zero.\n" +
			"- not/a/path\n",
		"indented paragraph after a blank line": "- **owned_paths:**\n" +
			"  - src/ — a wrapped\n    annotation\n" +
			"\n  A second indented paragraph, not a wrap.\n" +
			"  - not/a/path\n",
	} {
		got := ParseOwnedPaths(content)
		if len(got) != 1 || got[0] != "src/" {
			t.Errorf("%s: parse = %v, want [src/]", name, got)
		}
	}
}
