...
func GenerateKickoff(cfg *config.Config, role string) string {
	...
	b.WriteString("git status                        # check for uncommitted changes\n")
	b.WriteString("metis status                      # quick orientation\n")
	b.WriteString("```\n\n")
	b.WriteString("**Wrong branch:** STOP. Report to human.\n\n")
	b.WriteString("**Clean tree:** Continue to Step 2.\n\n")
	b.WriteString("**Dirty tree (uncommitted changes):** Attempt to resume:\n\n")
	b.WriteString("1. Run `metis next` to identify the active slice.\n")
	b.WriteString("2. If a brief exists (`metis brief <id>`), check whether the dirty files\n")
	b.WriteString("   fall within the brief's declared `owned_paths`:\n")
	b.WriteString("   - **Files are in scope:** This is a resumed/interrupted session.\n")
	b.WriteString("     Read the brief, review `git log --oneline` for progress so far,\n")
	b.WriteString("     and continue implementation from where it was left off.\n")
	b.WriteString("   - **Files are outside scope:** STOP. Report to human:\n")
	b.WriteString("     \"Dirty tree with out-of-scope files. Cannot safely resume.\"\n")
	b.WriteString("3. If no brief exists yet, check whether dirty files are only in `.metis/`:\n")
	b.WriteString("   - **Yes:** Safe to continue (partial brief or template in progress).\n")
	...
	if role == "" || role == "reviewer" {
		...
		b.WriteString("3. **Audit scope** — `metis log <id> --validate` (format + files vs brief scope; FAIL -> block)\n")
